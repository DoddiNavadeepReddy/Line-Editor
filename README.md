...
- **G V Raghuveer** — Document struct, core data structure (`init_document`, `insert_line`, `free_document`), search, find & replace, line/word count, help, undo command
- **D Navadeep** — `delete_line`, `display`, command parsing / user interaction, invalid line-number handling
...
- Undo last action (single-level, snapshot-based)
...
```bash
gcc -Wall -Wextra -std=c11 -o line_editor main.c document.c editor.c file.c undo.c
```
